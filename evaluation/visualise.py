...
def plot_emm_rt_bambi(df: pd.DataFrame, model: bmb.Model, results: az.InferenceData, figures_dir: str, show: bool = True) -> None:
...
def plot_emm_rt_statsmodels(df: pd.DataFrame, model: mlm.MixedLM, figures_dir: str, show: bool = True) -> None:
    predict_df = df[['soa', 'relationship']].drop_duplicates().copy()
    predict_df['participant'] = 'p1' # Dummy participant for prediction
...
def plot_emm_response(df: pd.DataFrame, model: bmb.Model, results: az.InferenceData, figures_dir: str, show: bool = True) -> None:
    predict_df = df[['soa', 'relationship']].drop_duplicates().copy()
    predict_df['participant'] = 'p1' # Dummy participant for prediction
